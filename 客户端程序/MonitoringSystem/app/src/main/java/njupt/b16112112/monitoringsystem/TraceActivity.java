...
public class TraceActivity extends Activity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.search_trace);
    ...
    private void init() {
            ...
            aMap = mapView.getMap();
            LatLng latLng = new LatLng(32.080728,118.77037);//构造一个位置
    ...
    private void initMoveMarker(){
        if(len == 0) Toast.makeText(TraceActivity.this, "无轨迹信息", Toast.LENGTH_LONG).show();
        else {
                            ...
                            runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    if (infoWindowLayout != null && title != null && moveMarker.getMarker().isInfoWindowShown()) {
                                        title.setText("距离所查询的终点还有： " + (int) distance + "米");
                                    }
                                    if (distance == 0) {
    ...
    private View getInfoWindowView(Marker marker) {
        if (infoWindowLayout == null) {
            infoWindowLayout = new LinearLayout(TraceActivity.this);
            infoWindowLayout.setOrientation(LinearLayout.VERTICAL);
            title = new TextView(TraceActivity.this);
            snippet = new TextView(TraceActivity.this);
